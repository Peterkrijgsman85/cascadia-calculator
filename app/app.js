  ...
  try {
    ...
    if (saved) {
      ...
      if (parsed && Array.isArray(parsed.players) && parsed.players.length > 0) {
        state = parsed;
        state.showDetails = false; // Altijd netjes inklappen bij herstart
        return;
      }
    }
  } catch (e) {
    console.error("State herstel mislukt, we starten een schone sessie.", e);
  }
...
function resetGame() {
  // Bevestiging vragen als er al daadwerkelijk scores zijn ingevoerd tijdens het spelen
  if (Object.keys(state.scores).length > 0 && state.mode === "game") {
    if (!confirm("Weet je zeker dat je dit spel wilt afbreken en opnieuw wilt beginnen?")) {
      return;
    }
  }
  
  localStorage.removeItem("cascadia_state_v4");
  state = JSON.parse(JSON.stringify(defaultState));
  save();
  render();
}

// Laad direct de juiste state in het geheugen
loadState();
...
// Start de initiële render zodra de DOM klaar is
